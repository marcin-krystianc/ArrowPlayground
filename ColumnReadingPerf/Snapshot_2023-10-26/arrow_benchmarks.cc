...
namespace
{
  ...
  std::shared_ptr<arrow::Table> GetTable(size_t nColumns, size_t nRows)
  {
    ...
    for (int i = 0; i < nColumns; i++)
    {
      ...
      fields.push_back(arrow::field("c_" + std::to_string(i), arrow::float64(), true));
    }
  ...
  Status RunMain(int argc, char **argv)
  {
    ...
    csvFile << "name,columns,rows,row_groups,columns_to_read,reading(μs),reading_p1(μs),reading_p2(μs)" << std::endl;

    std::list<int> nColumns = {100, 500, 5000};
    std::list<int64_t> row_groups_list = {1, 2, 5, 10, 20, 50, 100, 200};
    std::list<int> rows_list = {200000};

    std::vector<int> indicies(100);
    std::iota(indicies.begin(), indicies.end(), 0);

    for (auto row_groups : row_groups_list)
    {
      for (int nRow : rows_list)
      {
        for (int nColumn : nColumns)
        {
          auto chunk_size = nRow / row_groups;

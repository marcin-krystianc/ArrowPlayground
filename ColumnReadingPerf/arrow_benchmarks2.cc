...
arrow::Status ReadMetadata_CustomThrift(const std::string &filename)
{
  ...
  for (auto i = 0; i < 100; i++)
  {
    ...
    std::cerr << std::endl;
    std::cerr << " Deserialize full data: " << dt.count() << "ms" << std::endl;

    std::vector<size_t> columns_to_copy(2000);
    std::iota (std::begin(columns_to_copy), std::end(columns_to_copy), 0); // Fill with 0, 1, ..., 99.
    ...
    for (auto row_group = 0; row_group < row_groups; row_group++)
    {
      ...
      index_src = row_group_offset + gParserData.column_chunks[(columns + 1) * row_group + columns];
    }
    ...
    dt = std::chrono::duration_cast<std::chrono::milliseconds>(end - begin);
    std::cerr << " Trimming data: " << dt.count() << "ms" << std::endl;

    // try it
    begin = std::chrono::steady_clock::now();
    auto metadata2 = DeserializeFileMetadata((const void *)&buffer_dst[0], index_dst);
    end = std::chrono::steady_clock::now();
    dt = std::chrono::duration_cast<std::chrono::milliseconds>(end - begin);
    std::cerr << " Deserialize trimmed data: " << dt.count() << "ms" << std::endl;
  }
